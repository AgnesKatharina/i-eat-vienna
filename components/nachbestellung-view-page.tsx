...
export function NachbestellungViewPage({ nachbestellungId }: NachbestellungViewPageProps) {
  ...
  const products = nachbestellung.items.filter((item) => item.item_type === "product")
  ...
  return (
    <div className="container mx-auto p-4 sm:p-6 max-w-4xl">
      <div className="border border-gray-400 rounded-lg p-4 sm:p-6">
        ...
        {/* Products Section - Using is_packed for green color */}
        {products.length > 0 && (
          <Card className="mb-6">
            <CardHeader>
                ...
                {products.map((product) => (
                  ...
                  >
                    <div className="w-full text-left">
                      <h4 className="font-medium text-sm mb-1">{product.item_name}</h4>
                      <p className="text-xs opacity-80 mb-1">
                        {product.quantity} {product.unit}
                      </p>
                      {product.category && <p className="text-xs opacity-70">{product.category}</p>}
                    </div>
                  </Button>
                ))}
              </div>
        ...
        {/* Ingredients Section - Using is_packed for green color */}
            ...
            <CardContent>
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
                {ingredients.map((ingredient) => (
                  ...
                  >
                    <div className="w-full text-left">
                      <h4 className="font-medium text-sm mb-1">{ingredient.item_name}</h4>
                      <p className="text-xs opacity-80 mb-1">
                        {ingredient.quantity} {ingredient.unit}
                      </p>
                      {ingredient.category && <p className="text-xs opacity-70">{ingredient.category}</p>}
                    </div>
                  </Button>
                ))}
              </div>
